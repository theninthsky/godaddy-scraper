...
const domainNames = JSON.parse(
  fs.readFileSync(`${__dirname}/lib/domain-names.json`, 'utf8')
)
...
const startScraping = async () => {
  ...
  if (!fs.existsSync('results.csv')) {
    fs.writeFileSync('results.csv', 'Index, Domain (.com), Price ($)\n')
  }
  fs.appendFile(
    'results.csv',
  ...
  console.log(
    `\n\nCompleted. Results are saved to file ${startGreenColor}results.csv${endColor}.\n`
  )
}
